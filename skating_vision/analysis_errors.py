...
def classify_video_failure(exc: Exception) -> AnalysisFailure:
    ...
    decode_tokens = (
        ...
        "unsupported codec", "could not open input file", "input/output error",
    )
    code = AnalysisErrorCode.VIDEO_DECODE_FAILED if any(t in lowered for t in decode_tokens) else AnalysisErrorCode.FRAME_EXTRACT_FAILED
...
def classify_ai_failure(exc: Exception) -> AnalysisFailure:
    ...
    elif isinstance(exc, BadRequestError) and any(t in lowered for t in ("content_filter", "content filter", "safety", "审核")):
        code = AnalysisErrorCode.AI_API_CONTENT_FILTER
    elif any(t in lowered for t in ("content_filter", "content filter", "safety system", "safety policy", "审核拦截")):
        code = AnalysisErrorCode.AI_API_CONTENT_FILTER
    elif any(t in lowered for t in ("api key", "api_key", "secret_key", "尚未配置 api key", "未找到 slot", "尚未配置")):
        code = AnalysisErrorCode.AI_API_AUTH_ERROR
    else:
