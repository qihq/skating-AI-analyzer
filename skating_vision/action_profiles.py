...
def _max_vertical_range(pose_data: dict[str, Any] | None) -> float:
    ...
    for frame in frames:
        ...
        if not isinstance(keypoints, list):
            continue
        visible = [
            p for p in keypoints
            if isinstance(p, dict) and float(p.get("visibility", 0.0) or 0.0) >= 0.5 and p.get("id") in {11, 12, 23, 24}
        ]
        if not visible:
            continue
        points.append(sum(float(p.get("y", 0.0) or 0.0) for p in visible) / len(visible))
...
def infer_analysis_profile(
    ...
    evidence = {
        "action_type": action_type, "action_subtype": subtype, "profile_hint": hinted_profile,
        "max_motion_score": round(max_motion, 4), "avg_motion_score": round(avg_motion, 4),
        "com_vertical_range": round(vertical_range, 4), "jump_gate_passed": jump_gate,
        "spiral_gate_passed": spiral_gate, "negative_constraints": [],
    }
