...
**建议思考等级**: `low | medium | high | xhigh`

等级含义：

- `low`: 小范围接线、类型补充、简单 UI/API 展示。
- `medium`: 单模块实现，有清晰输入输出和测试。
- `high`: 涉及流水线、多模块集成、兼容旧行为。
- `xhigh`: 核心算法、融合策略、复杂边界条件，建议深度推理。
...
- `P1-01`: 关键帧候选检测，涉及运动学规则和边界场景。
- `P3-02`: 加权融合服务，是整体正确率提升的核心策略。
- `P3-03`: 融合接入现有 vision 流水线，容易影响兼容性。
- `P4-05`: 低质量视频保守策略，直接影响误判和报告可信度。
