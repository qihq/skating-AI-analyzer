"""Path B: 骨架叠加帧 + 生物力学数值 grounding 分析。

职责: 将标注帧和逐帧生物力学数值注入 LLM prompt，进行量化分析。
输入: 标注帧图片 + bio_context 逐帧测量值。
输出: frame_analysis + subscores + top_issues。
"""
    ...
    *,
    ...
    try:
        ...
        user_text = _build_user_prompt(
            action_type, action_subtype, analysis_profile, profile_evidence, jump_metrics_text, n_frames,
        )
