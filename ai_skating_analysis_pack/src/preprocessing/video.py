...
- 运动密度曲线计算（OpenCV 帧差法）
- 动作窗口检测（滑动窗口峰值定位）
- 帧编码为 base64 data_url 供 LLM 视觉分析
"""
...
PROCESSING_ROOT = Path("/tmp/skating-analyzer") if Path("/tmp").exists() else Path("./_processing")
...
def build_upload_paths(analysis_id: str, suffix: str) -> tuple[Path, Path]:
    upload_dir = PROCESSING_ROOT / analysis_id
...
async def extract_frames(video_path: Path, frames_dir: Path) -> list[Path]:
    ...
    await _run_ffmpeg(
        ["-y", "-i", str(video_path), "-t", str(MAX_SECONDS),
         "-vf", scale_filter, "-pix_fmt", "yuvj420p", output_pattern]
    )
...
async def _extract_action_thumbnails(video_path: Path, thumbs_dir: Path) -> list[Path]:
    ...
    await _run_ffmpeg(
        ["-y", "-i", str(video_path), "-vf",
         f"fps={ACTION_WINDOW_DETECTION_FPS},scale=160:90", output_pattern]
    )
...
async def _extract_thumbnails_in_window(video_path: Path, thumbs_dir: Path, start_sec: float, end_sec: float) -> list[Path]:
    ...
    await _run_ffmpeg(
        ["-y", "-ss", f"{start_sec:.3f}", "-to", f"{end_sec:.3f}",
         "-i", str(video_path), "-vf", scale_filter, "-pix_fmt", "yuvj420p", output_pattern]
    )
    return sorted(thumbs_dir.glob("thumb_*.jpg"))
...
async def _extract_full_frame_at(video_path: Path, timestamp: float, target_path: Path) -> None:
    ...
    await _run_ffmpeg(
        ["-y", "-ss", f"{timestamp:.3f}", "-i", str(video_path),
         "-frames:v", "1", "-vf", scale_filter, "-pix_fmt", "yuvj420p", str(target_path)]
    )
...
async def extract_motion_sampled_frames(
    ...
    try:
        ...
        logger.warning("Thumbnail extraction inside action window failed for %s, falling back to full-video sampling", video_path, exc_info=True)
        if thumbs_dir.exists():
            shutil.rmtree(thumbs_dir, ignore_errors=True)
    ...
    try:
        ...
        logger.warning("Full-frame extraction failed for %s, falling back to uniform frame extraction", video_path, exc_info=True)
        for existing_frame in frames_dir.glob("frame_*.jpg"):
            existing_frame.unlink(missing_ok=True)
        output_paths = sample_frame_paths(await extract_frames(video_path, frames_dir), MAX_SAMPLED_FRAMES)
        selected_records = [
            {"frame_id": frame_path.stem, "source_thumb_index": index,
             "timestamp": round(index / FRAME_RATE, 3), "motion_score": None}
