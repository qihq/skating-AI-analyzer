...
职责：
- 从骨骼关键点序列计算膝关节角度、躯干倾斜、手臂对称性
- 质心（CoM）轨迹追踪
- T/A/L 关键帧检测（起跳/顶点/落冰）
- 跳跃指标估算（滞空时间、高度、起跳速度、转速）
- 旋转轴稳定性评估
- 子维度评分（起跳发力/旋转轴心/手臂配合/落冰缓冲/核心稳定）
"""
...
def _empty_jump_metrics() -> dict[str, Any]:
    return {"air_time_seconds": None, "estimated_height_cm": None, "takeoff_speed_mps": None,
            "rotation_rps": None, "estimated_rotations": None, "probable_jump_type": "unknown"}


def _empty_analysis(knee_angles=None, trunk_tilts=None, arm_symmetry=None, *, analysis_profile="jump") -> dict[str, Any]:
    return {
        "analysis_profile": analysis_profile,
        "knee_angles": knee_angles or [], "trunk_tilts": trunk_tilts or [], "arm_symmetry": arm_symmetry or [],
        "com_trajectory": {"points": [], "vertical_range": 0},
        "rotation_stability": {"average_tilt_degrees": None, "stability_score": 65},
        "bio_subscores": {"takeoff_power": 65, "rotation_axis": 65, "arm_coordination": 65,
                          "landing_absorption": 65, "core_stability": 65},
        "discipline_metrics": {}, "quality_flags": [], "key_frames": {},
...
def _shoulder_hip_reference_length(keypoints: list[dict[str, Any]]) -> float:
    left_shoulder, right_shoulder = _point(keypoints, 11), _point(keypoints, 12)
    left_hip, right_hip = _point(keypoints, 23), _point(keypoints, 24)
    if not all((left_shoulder, right_shoulder, left_hip, right_hip)):
        return 0.0
    return _distance(_midpoint(left_shoulder, right_shoulder), _midpoint(left_hip, right_hip))
...
def calc_knee_angle(keypoints: list[dict[str, Any]], frame_idx: int) -> dict[str, Any]:
    ...
    return {"frame_idx": frame_idx, "left": left_angle, "right": right_angle,
            "min_angle": min(values) if values else None}
...
def calc_arm_symmetry(keypoints: list[dict[str, Any]], frame_idx: int) -> dict[str, Any]:
    ...
    left_shoulder, right_shoulder = _point(keypoints, 11), _point(keypoints, 12)
    left_wrist, right_wrist = _point(keypoints, 15), _point(keypoints, 16)
...
def calc_center_of_mass_trajectory(pose_data: dict[str, Any]) -> dict[str, Any]:
    ...
    for frame in pose_data.get("frames", []):
        ...
        hips = [_point(keypoints, 23), _point(keypoints, 24)]
        shoulders = [_point(keypoints, 11), _point(keypoints, 12)]
        visible = [p for p in hips + shoulders if p is not None]
        if not visible:
            continue
        y_value = sum(p["y"] for p in visible) / len(visible)
        points.append({"frame": frame.get("frame", ""), "x": sum(p["x"] for p in visible) / len(visible), "y": y_value})
        y_values.append(y_value)
        ref_len = _shoulder_hip_reference_length(keypoints)
        if ref_len >= 0.01:
            reference_lengths.append(ref_len)
    vertical_range = ((max(y_values) - min(y_values)) / (sum(reference_lengths) / len(reference_lengths))
                      if y_values and reference_lengths else 0.0)
...
def _find_descent_start(points, apex_index):
...
def _find_ascent_start(points, apex_index):
...
def _find_max_hip_x_delta(frames):
    best_index, best_delta, previous_hip = None, 0.0, None
...
def _free_leg_ankle_y(frame):
    keypoints = frame.get("keypoints", [])
    ankles = [_point(keypoints, 27), _point(keypoints, 28)]
    visible = [p["y"] for p in ankles if p is not None]
    return min(visible) if visible else None


def _find_free_leg_peak(frames):
    best_index, best_y = None, None
    for index, frame in enumerate(frames):
        ankle_y = _free_leg_ankle_y(frame)
        if ankle_y is None:
            continue
        if best_y is None or ankle_y < best_y:
            best_y, best_index = ankle_y, index
    return best_index


def detect_key_frames(com_trajectory, pose_data, analysis_profile="jump"):
    points = com_trajectory.get("points", [])
    if len(points) < 3:
        return {}
    if analysis_profile == "jump":
        apex_index = min(range(len(points)), key=lambda i: points[i]["y"])
        takeoff_index = _find_descent_start(points, apex_index)
        landing_index = _find_ascent_start(points, apex_index)
        return {"T": PathLikeFrame(points[takeoff_index]["frame"]).stem,
    ...
    if analysis_profile == "spin":
        ...
        if max_delta_index is None:
            return {}
        start_index, end_index = max(0, max_delta_index - 1), min(len(frames) - 1, max_delta_index + 1)
        return {"旋转入": PathLikeFrame(str(frames[start_index].get("frame", ""))).stem,
                "旋转中": PathLikeFrame(str(frames[max_delta_index].get("frame", ""))).stem,
                "旋转出": PathLikeFrame(str(frames[end_index].get("frame", ""))).stem}
    if analysis_profile in ("spiral", "step"):
        peak_index = _find_free_leg_peak(frames)
        if peak_index is None:
            return {}
        return {"峰值": PathLikeFrame(str(frames[peak_index].get("frame", ""))).stem}
    return {}


def calc_rotation_axis_stability(pose_data, start_frame, end_frame):
    tilts = []
    for frame in pose_data.get("frames", []):
...
def _normalized_angle_delta(current_angle, previous_angle):
...
def _rotation_rps(pose_data, start_frame, end_frame):
    angles = []
    for frame in pose_data.get("frames", []):
        frame_idx = _frame_number(str(frame.get("frame", "")))
        if start_frame <= frame_idx <= end_frame:
    ...
    if len(angles) < 2:
        return 0.0
    total_rotation = sum(abs(_normalized_angle_delta(a, b)) for a, b in zip(angles[1:], angles))
    total_turns = total_rotation / (2 * math.pi)
    duration = max((end_frame - start_frame) / FPS, 1 / FPS)
    return round(total_turns / duration, 2)


def estimate_jump_rotations(rotation_rps, air_time_seconds):
    ...
    thresholds = [(0.8, 1.8, "单圈跳 (1T/1S/1Lo/1F/1Lz)"), (1.8, 2.8, "双圈跳 (2A/2T/2S/2Lo/2F/2Lz)"),
                  (2.8, 3.8, "三圈跳 (3A/3T/3S/3Lo/3F/3Lz)"), (3.8, 5.0, "四圈跳 (4T/4S/4Lo/4F/4Lz)")]
    probable = "unknown"
    for low, high, label in thresholds:
        if low <= rotations < high:
            probable = label
            break
    return {"estimated_rotations": round(rotations, 2), "probable_jump_type": probable}
...
def _invalid_metrics(reason):
    return {"jump_metrics": _empty_jump_metrics(), "jump_metrics_status": "invalid", "jump_metrics_warning": reason}


def sanitize_biomechanics_data(bio_data):
    ...
    if not isinstance(metrics, dict):
        ...
        return sanitized
    metric_values = {"air_time_seconds": _to_float(metrics.get("air_time_seconds")),
    ...
    warning = None
    if not isinstance(key_frames, dict) or not all(key_frames.get(l) for l in ("T", "A", "L")):
        warning = "关键帧检测异常"
    elif any(v is None for v in metric_values.values()):
        warning = "关键指标缺失"
    else:
        at, ht, ts, rt = metric_values["air_time_seconds"], metric_values["estimated_height_cm"], metric_values["takeoff_speed_mps"], metric_values["rotation_rps"]
        if at is None or at <= 0 or at > MAX_AIR_TIME_SECONDS:
            warning = "滞空时间检测异常"
        elif ht is None or ht <= 0 or ht > MAX_HEIGHT_CM:
            warning = "跳跃高度检测异常"
        elif ts is None or ts <= 0 or ts > MAX_TAKEOFF_SPEED_MPS:
            warning = "起跳速度检测异常"
        elif rt is None or rt <= 0 or rt > MAX_ROTATION_RPS:
            warning = "转速检测异常"
    if warning:
    ...
    sanitized["jump_metrics"] = {
        ...
        **estimate_jump_rotations(metric_values["rotation_rps"], metric_values["air_time_seconds"])}
    sanitized["jump_metrics_status"] = "ok"
    sanitized["jump_metrics_warning"] = None
    return sanitized


def _score_from_values(values, ideal, tolerance, invert=False):
    if not values:
        return 65
    average = sum(values) / len(values)
    score = 100 - (abs(average - ideal) / tolerance) * 35
    if invert:
        score = 100 - average
    return max(40, min(100, round(score)))


def _spiral_discipline_metrics(trunk_tilts, knee_angles, arm_symmetry, com_trajectory):
...
def _non_jump_bio_subscores(discipline_metrics, arm_symmetry):
    symmetries = [float(item["symmetry"]) for item in arm_symmetry if item.get("symmetry") is not None]
    arm_score = max(40, min(100, round((sum(symmetries) / len(symmetries)) * 100))) if symmetries else 65
    gs = _to_float(discipline_metrics.get("glide_stability"))
    sls = _to_float(discipline_metrics.get("support_leg_stability"))
    hsa = _to_float(discipline_metrics.get("hip_shoulder_alignment"))
    rotation_axis = round(gs) if gs is not None else 65
    landing_absorption = round(sls) if sls is not None else 65
    core_stability = round(hsa) if hsa is not None else rotation_axis
    tpv = [v for v in (gs, sls) if v is not None]
    takeoff_power = round(sum(tpv) / len(tpv)) if tpv else 65
    return {"takeoff_power": max(0, min(100, takeoff_power)), "rotation_axis": max(0, min(100, rotation_axis)),
            "arm_coordination": max(0, min(100, arm_score)), "landing_absorption": max(0, min(100, landing_absorption)),
...
def analyze_biomechanics(pose_data: dict[str, Any], action_type: str, analysis_profile: str = "jump") -> dict[str, Any]:
    ...
    if analysis_profile != "jump":
        ...
        return sanitize_biomechanics_data({
            "analysis_profile": analysis_profile, "knee_angles": knee_angles, "trunk_tilts": trunk_tilts,
            "arm_symmetry": arm_symmetry, "com_trajectory": com_trajectory,
            "rotation_stability": {"average_tilt_degrees": None, "stability_score": 65},
            "bio_subscores": bio_subscores, "discipline_metrics": discipline_metrics,
            "quality_flags": [], "key_frames": key_frames,
            "jump_metrics": None, "jump_metrics_status": "not_applicable", "jump_metrics_warning": None})
    ...
    return sanitize_biomechanics_data({
        "analysis_profile": analysis_profile, "knee_angles": knee_angles, "trunk_tilts": trunk_tilts,
        "arm_symmetry": arm_symmetry, "com_trajectory": com_trajectory,
        "rotation_stability": rotation_stability, "bio_subscores": bio_subscores,
        "discipline_metrics": {}, "quality_flags": [], "key_frames": key_frames,
        "jump_metrics": {"air_time_seconds": air_time_seconds, "estimated_height_cm": estimated_height_cm,
