...
- 生成结构化训练报告（summary/issues/improvements/training_focus/subscores）
- 子维度融合评分（AI 40% + 生物力学 60%）
- 综合发力评分计算
"""
...
SUBSCORE_WEIGHTS = {
    "takeoff_power": 0.25, "rotation_axis": 0.25, "arm_coordination": 0.15,
    "landing_absorption": 0.25, "core_stability": 0.10,
}
...
def _fallback_subscores() -> dict[str, int]:
    return {"takeoff_power": 75, "rotation_axis": 75, "arm_coordination": 75, "landing_absorption": 75, "core_stability": 75}
...
def calculate_force_score(report: dict[str, Any]) -> int:
    ...
    penalties = {Severity.high.value: 15, Severity.medium.value: 8, Severity.low.value: 3}
    score = 100
...
def normalize_report(payload: dict[str, Any], bio_data: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for issue in payload.get("issues", []):
        ...
        normalized_issues.append({
            ...
            "severity": severity, "phase": str(issue.get("phase", "")).strip() or None,
            "frames": [str(f) for f in frames] if isinstance(frames, list) else [],
        })
...
async def generate_report(
    ...
    raw_content = await request_text_completion(
        provider, temperature=0.25, max_tokens=1800,
        messages=[
    ...
    try:
        ...
        raise AnalysisPipelineError(
            ...
            f"Report JSON parse failed: {exc}: {cleaned[:500]}") from exc
