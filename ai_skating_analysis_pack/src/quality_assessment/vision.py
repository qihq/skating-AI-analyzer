...
职责：
- 将抽帧图片（base64）+ 结构化 prompt 发送给多模态 LLM
- 获取逐帧 phase/observations/issues/positives 结构化输出
- 归一化和校验 LLM 返回的 JSON
"""
...
from src.utils.analysis_errors import AnalysisErrorCode, AnalysisPipelineError
from src.utils.providers import extract_message_text, get_active_provider
from src.quality_assessment.report import clean_json_text
from src.utils.snowball import build_memory_context
from src.preprocessing.video import FramePayload
...
def normalize_vision_payload(payload: dict[str, Any], frame_payloads: list[FramePayload]) -> dict[str, Any]:
    ...
    detected_phases = [str(phase) for phase in summary.get("detected_phases", [])
                       if str(phase) in VALID_PHASES and str(phase) != "不可分析"]
    return {
        "frame_analysis": frame_analysis,
        "action_phase_summary": {"detected_phases": detected_phases,
...
async def analyze_frames(
    ...
    *,
    ...
    response = await client.chat.completions.create(
        model=provider.model_id, temperature=0.1, max_tokens=3500, extra_body=extra_body,
    ...
    try:
        ...
        raise AnalysisPipelineError(
            ...
            f"Vision JSON parse failed: {exc}: {cleaned[:500]}") from exc
