...
职责：
- 对抽帧图片逐帧提取 33 个 MediaPipe Pose 关键点
- 支持单人（fallback）和多人（Tasks API）两种模式
- 结合 target_lock bbox 进行裁剪，聚焦主滑行者
- 输出归一化坐标 (0~1) 的关键点序列
"""
...
from src.preprocessing.target_lock import extract_pose_target_bbox
...
def get_pose_runtime_status() -> dict[str, Any]:
    ...
    return {"mode": mode, "configured": configured, "model_path": task_model_path or None,
            "model_exists": exists, "num_poses": num_poses, "reason": reason}
...
def _bbox_from_landmarks(landmarks: list[Any]) -> dict[str, float] | None:
    ...
    return {"x": round(left, 4), "y": round(top, 4),
            "width": round(max(0.05, right - left), 4), "height": round(max(0.05, bottom - top), 4)}


def _bbox_center(bbox: dict[str, float] | None) -> tuple[float, float]:
    if not bbox:
        return 0.5, 0.5
    return (float(bbox.get("x", 0.0)) + float(bbox.get("width", 0.0)) / 2,
...
def _map_landmarks_to_keypoints(
    landmarks: list[Any], *, crop_left: int, crop_top: int,
    crop_width: int, crop_height: int, image_width: int, image_height: int,
) -> list[dict[str, Any]]:
    ...
    for index, landmark in enumerate(landmarks):
        ...
        keypoints.append({
            ...
            "x": float(normalized_x), "y": float(normalized_y),
            "z": float(landmark.z), "visibility": visibility if visibility >= 0.5 else 0.0,
        })
    return keypoints
...
def _score_candidate(bbox, visibility_sum, previous_bbox, motion_bbox) -> float:
    if not bbox:
        return -1.0
    ...
    return round(
        (iou_score * 0.34) + (continuity_score * 0.22) + (scale_score * 0.14)
        + (visibility_score * 0.14) + (motion_overlap * 0.16), 4,
    )
...
def log_pose_runtime_mode() -> None:
    ...
    elif status["reason"] == "missing_model_file":
...
def extract_pose(frames_dir: str, target_lock: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        for frame_path in frame_paths:
            ...
            if image is None:
                frames.append({"frame": frame_path.name, "keypoints": [], "target_bbox": previous_bbox,
                               "tracking_confidence": 0.0, "tracking_state": "lost"})
                continue
                ...
                try:
                    ...
                    for landmarks in detect_result.pose_landmarks:
                        ...
                        candidate_results.append({
                            "bbox": bbox, "visibility_sum": _visibility_sum(landmarks),
                            "keypoints": _map_landmarks_to_keypoints(
                                landmarks, crop_left=0, crop_top=0,
            ...
            if not candidate_results:
                ...
                if cropped.size > 0:
                    ...
                    if result.pose_landmarks:
                        ...
                        candidate_results.append({
                            "bbox": bbox, "visibility_sum": _visibility_sum(result.pose_landmarks.landmark),
                            "keypoints": _map_landmarks_to_keypoints(
                                result.pose_landmarks.landmark, crop_left=left, crop_top=top,
                                crop_width=max(right - left, 1), crop_height=max(bottom - top, 1),
                                image_width=image_width, image_height=image_height),
                            "source": "single_pose_crop",
                        })

            scored_candidates = [{**candidate, "score": _score_candidate(
                candidate.get("bbox"), float(candidate.get("visibility_sum", 0.0)),
                previous_bbox, motion_bbox)} for candidate in candidate_results]
            ...
            if best_candidate is None:
                lost_count += 1
                frames.append({"frame": frame_path.name, "keypoints": [], "target_bbox": previous_bbox,
                               "tracking_confidence": 0.0, "tracking_state": "lost" if lost_count > 0 else "missing"})
                continue

            previous_bbox = best_candidate.get("bbox")
            lost_count = 0
            frames.append({
                "frame": frame_path.name, "keypoints": best_candidate.get("keypoints", []),
