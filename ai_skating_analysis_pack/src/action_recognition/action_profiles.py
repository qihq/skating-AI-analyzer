...
职责：
- 根据用户输入的动作类型/子类型推断分析 profile（jump/spin/step/spiral）
- 结合骨骼姿态数据和运动密度分数做几何验证
- 提供跳跃特征知识库（起跳刃、方向、识别要点）
"""
...
def _normalize_jump_subtype_key(action_subtype: str) -> str:
    return (action_subtype.lower().replace(" ", "").replace("_", "").replace("-", "")
            .replace("（", "(").replace("）", ")"))
...
def get_jump_characteristics(action_subtype: str | None) -> dict[str, str] | None:
    ...
    normalized_characteristics = [(_normalize_jump_subtype_key(key), ch) for key, ch in JUMP_CHARACTERISTICS.items()]
    for normalized_key, characteristics in normalized_characteristics:
        if normalized == normalized_key:
            return characteristics
    for normalized_key, characteristics in sorted(normalized_characteristics, key=lambda item: len(item[0]), reverse=True):
...
def _max_vertical_range(pose_data: dict[str, Any] | None) -> float:
    ...
    for frame in frames:
        ...
        if not isinstance(keypoints, list):
            continue
        visible = [point for point in keypoints if isinstance(point, dict)
                    and float(point.get("visibility", 0.0) or 0.0) >= 0.5 and point.get("id") in {11, 12, 23, 24}]
        if not visible:
            continue
...
def _person_height_normalized(pose_data: dict[str, Any] | None) -> float:
    ...
    for frame in frames:
        ...
        if not isinstance(keypoints, list):
            continue
        nose = next((p for p in keypoints if isinstance(p, dict) and p.get("id") == 0
                      and float(p.get("visibility", 0.0) or 0.0) >= 0.5), None)
        left_ankle = next((p for p in keypoints if isinstance(p, dict) and p.get("id") == 27
                           and float(p.get("visibility", 0.0) or 0.0) >= 0.5), None)
        right_ankle = next((p for p in keypoints if isinstance(p, dict) and p.get("id") == 28
                            and float(p.get("visibility", 0.0) or 0.0) >= 0.5), None)
        if nose is None or (left_ankle is None and right_ankle is None):
            continue
        ankles = [p for p in (left_ankle, right_ankle) if p is not None]
        ankle_y = sum(float(p.get("y", 0.0) or 0.0) for p in ankles) / len(ankles)
...
def _detect_airborne_frames(pose_data: dict[str, Any] | None) -> int:
    ...
    for frame in frames:
        ...
        if not isinstance(keypoints, list):
            continue
        ankles = [p for p in keypoints if isinstance(p, dict) and p.get("id") in {27, 28}
                   and float(p.get("visibility", 0.0) or 0.0) >= 0.4]
        if not ankles:
            continue
        ankle_y_series.append(sum(float(p.get("y", 0.0) or 0.0) for p in ankles) / len(ankles))
...
def _detect_rotation_signal(pose_data: dict[str, Any] | None) -> float:
    ...
    for frame in frames:
        ...
        if not isinstance(keypoints, list):
            continue
        hips = [p for p in keypoints if isinstance(p, dict) and p.get("id") in {23, 24}
                and float(p.get("visibility", 0.0) or 0.0) >= 0.4]
        if not hips:
            continue
        hip_x_series.append(sum(float(p.get("x", 0.0) or 0.0) for p in hips) / len(hips))
...
def _motion_stats(frame_motion_scores: dict[str, Any] | None) -> tuple[float, float]:
    if not isinstance(frame_motion_scores, dict):
        return 0.0, 0.0
    scores = [float(s) for s in frame_motion_scores.get("scores", []) if isinstance(s, (int, float))]
    if not scores:
        return 0.0, 0.0
...
def infer_analysis_profile(
    ...
    """推断分析 profile，返回 (profile_name, evidence_dict)。"""
    subtype = normalize_action_subtype(action_type, action_subtype)
    ...
    evidence = {
        "action_type": action_type, "action_subtype": subtype,
        "profile_hint": hinted_profile,
        "max_motion_score": round(max_motion, 4), "avg_motion_score": round(avg_motion, 4),
        ...
        "jump_gate_passed": jump_gate, "spiral_gate_passed": spiral_gate,
        "quality_flags": [], "negative_constraints": [],
    }
    ...
    if hinted_profile == "spin":
        if rotation_signal < 0.15:
            ...
            evidence["negative_constraints"].append(f"髋部旋转信号弱（{rotation_signal:.3f}），可能不是旋转或存在视角遮挡")
    ...
    if hinted_profile == "jump":
        evidence["negative_constraints"].append("几何证据不足（CoM 垂直范围低、无腾空帧检测），但用户填写了跳跃，保留 jump profile")
