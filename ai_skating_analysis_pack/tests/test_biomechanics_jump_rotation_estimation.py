...
from src.quality_assessment.biomechanics import analyze_biomechanics


def _jump_keypoints(shoulder_angle, com_shift):
    keypoints = [{"id": i, "x": 0.0, "y": 0.0, "z": 0.0, "visibility": 0.0} for i in range(33)]
    shoulder_center_x, shoulder_center_y = 0.5, 0.24 + com_shift
    shoulder_radius = 0.08
    hip_center_x, hip_center_y = 0.5, 0.54 + com_shift
    hip_half_width = 0.06
    left_shoulder = (shoulder_center_x - math.cos(shoulder_angle) * shoulder_radius,
                     shoulder_center_y - math.sin(shoulder_angle) * shoulder_radius)
    right_shoulder = (shoulder_center_x + math.cos(shoulder_angle) * shoulder_radius,
                      shoulder_center_y + math.sin(shoulder_angle) * shoulder_radius)
    visible_points = {
    ...
    for index, (x, y) in visible_points.items():
        keypoints[index] = {"id": index, "x": x, "y": y, "z": 0.0, "visibility": 0.99}
    return keypoints


class BiomechanicsJumpRotationEstimationTests(unittest.TestCase):
    def test_jump_metrics_include_estimated_rotations_and_probable_type(self):
        pose_data = {"frames": [
